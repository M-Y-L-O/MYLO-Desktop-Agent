...
class ArchitectureDescriptor:
    ...
    def _validate_shape_propagation(self):
        ...
        for node_id in sorted_nodes:
            ...
            # Update input parameters in-place to ensure downstream consistency
    ...
    def _merge_shapes(self, input_shapes: List[List[int]], node: Node) -> List[int]:
        merge_rule = self.merge_rules.get(node.id, "concat")
        if merge_rule == "concat":
            feature_dim = sum(shape[-1] for shape in input_shapes)
            batch = input_shapes[0][0] if input_shapes else -1
            prefix = input_shapes[0][:-1] if len(input_shapes[0]) > 1 else [batch]
            return prefix + [feature_dim]
